...
class Invaders(GameApp):
    ...
    #
    # Attribute _keypressed: determines whether keys were previously pressed
    # Invariant: _keypressed is an int, either 1 or 0... It is 1 if key was
    # previously pressed and 0 if key was not previously pressed
    ...
    def start(self):
        ...
        if self._state == STATE_INACTIVE:
            ...
            y= GAME_HEIGHT/2, font_name = "Arcade", font_size = 64)
        if self._state != STATE_INACTIVE:
            self._text = None
    ...
    def update(self,dt):
        ...
        """
        if(self._state == STATE_COMPLETE) and self._wave.lose():
            self._lose_message()
        if(self._state == STATE_COMPLETE) and self._wave.win():
            self._win_message()
        if (self._state == STATE_PAUSED or self._state == STATE_ACTIVE) \
        and self._wave.lose():
            self._lose_message()
            self._state = STATE_COMPLETE
        if (self._state == self._state == STATE_PAUSED \
        or self._state == STATE_ACTIVE) and self._wave.win():
            self._win_message()
            self._state = STATE_COMPLETE
        if self._state == STATE_INACTIVE:
            self._new_wave()
        ...
        if self._state ==  STATE_ACTIVE:
            self._wave.update(self.input,self._state,dt)
        if self._state != STATE_INACTIVE and self._wave.getReset():
    ...
    def draw(self):
        ...
        if self._state == STATE_PAUSED and self._wave.getLives() > 0:
            ...
            self._text.draw(self.view)
        elif self._wave != None:
            self._wave.draw(self.view)

    # HELPER METHODS FOR THE STATES GO HERE
    def _new_wave(self):
        """
        Helper to update method that starts a new wave of aliens if 's' has
        been pressed.

        Determines whether 's' key has been pressed and state == STATE_NEWWAVE
        and starts a new wave if so (by setting self._state to STATE_NEWWAVE).
        If 's' has been pressed it also sets self._text to None.
        """
        curr_keys = self.input.keys
        change = (len(curr_keys) > 0 and self._keypressed == 0)

        if change and self._state == STATE_INACTIVE:
            if 's' in curr_keys:
                self._state = STATE_NEWWAVE
                self._text = None
        if 's' in curr_keys and len(curr_keys) != 0:
            self._keypressed = 1
        if self._state == STATE_NEWWAVE:
            self._keypressed = 0
    ...
    def _create_wave(self):
        """
        Helper method to update method that creates a wave.

        Creates a Wave object and stores it inside of hidden attribute
        self._wave. It also sets self._state to STATE_ACTIVE.
        """
        self._wave = Wave()
        self._state = STATE_ACTIVE
    ...
    def _continue_wave(self):
        """
        Helper method to update method that indicates continuation of the wave.

        Determines whether 's' key has been pressed and if self._state ==
        STATE_PAUSED. This method sets self._state to STATE_CONTINUE and
        self._text to None if the previous conditions were met. Otherwise,
        nothing new occurs and all attributes remain unchanged.
        """
        curr_keys = self.input.keys
        change = (len(curr_keys) > 0 and self._keypressed == 0)
    ...
    def _lose_message(self):
        """
        Sets self._text to a message saying 'You Lost, Loser.'
        """
        self._text = GLabel(text= "You Lost, Loser.", x = GAME_WIDTH/2, \
        y= GAME_HEIGHT/2, font_name = "Arcade", font_size = 64)

    def _win_message(self):
        """
        Sets self._text to a message saying 'You Won.'
        """
        self._text = GLabel(text= "You Won.", x = GAME_WIDTH/2, \
        y= GAME_HEIGHT/2, font_name = "Arcade", font_size = 64)
